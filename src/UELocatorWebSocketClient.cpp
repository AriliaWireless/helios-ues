...
namespace OpenWifi {
    ...
    UELocatorWebSocketClient::UELocatorWebSocketClient(Poco::Logger &Logger):
		 Logger_(Logger){
		UI_WebSocketClientServer()->SetProcessor(this);
	}

    UELocatorWebSocketClient::~UELocatorWebSocketClient() {
		UI_WebSocketClientServer()->SetProcessor(nullptr);
	}

	void UELocatorWebSocketClient::Processor(const Poco::JSON::Object::Ptr &O, std::string &Answer, bool &Done, [[maybe_unused]] const SecurityObjects::UserInfo &UserInfo ) {
		try {
			if (O->has("command")) {
	...
	void UELocatorWebSocketClient::ws_command_serial_number_search(const Poco::JSON::Object::Ptr &O,
	...
	void UELocatorWebSocketClient::ws_command_exit([[maybe_unused]] const Poco::JSON::Object::Ptr &O, bool &Done, std::string &Answer) {
		Done = true;
		Answer = R"lit({ "closing" : "Goodbye! Aurevoir! Hasta la vista!" })lit";
	}

	void UELocatorWebSocketClient::ws_command_invalid([[maybe_unused]] const Poco::JSON::Object::Ptr &O, bool &Done, std::string &Answer) {
		Done = false;
