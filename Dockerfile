...
FROM build-base AS ues-build

ADD CMakeLists.txt build /helios-ues/
ADD cmake /helios-ues/cmake
ADD src /helios-ues/src
ADD .git /helios-ues/.git
...
WORKDIR /helios-ues
RUN mkdir cmake-build
WORKDIR /helios-ues/cmake-build
RUN cmake ..
RUN cmake --build . --config Release -j8
...
ENV OWUES_USER=owgw \
    OWUES_ROOT=/owgw-data \
    OWUES_CONFIG=/owgw-data

RUN useradd "$OWUES_USER"

RUN mkdir /helios
RUN mkdir -p "$OWUES_ROOT" "$OWUES_CONFIG" && \
    chown "$OWUES_USER": "$OWUES_ROOT" "$OWUES_CONFIG"
...
COPY --from=ues-build /helios-ues/cmake-build/ues /helios/ues
